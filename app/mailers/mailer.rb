class Mailer < ApplicationMailer
  ...
  def rejection_message_for_receiver(direct_message)
    @direct_message = direct_message
    @receiver = @direct_message.receiver
    @email_to = @receiver.email
    manage_subscriptions_token(@receiver)
    admin_emails = User.where(official_position: 'Admin').pluck(:email)
    with_user(@receiver) do
      mail(to: @email_to, subject: t("mailers.rejection_message_for_receiver.subject"), bcc: admin_emails )
    end
  end
  ...
  def send_verification_email(user)
    ...
    with_user(@user) do
      mail(to: @email_to, subject: t("mailers.verified_account.subject"))
    end
  end
  private
